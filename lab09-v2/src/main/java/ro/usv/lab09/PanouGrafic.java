    ...
    private HBox operatiiArbore() {
        ...
        btnSave.setOnAction(e -> {
            ...
            try {
                ...
                ArrayList<Nod<String>> arr = arb.SRD();
                nr = arr.size();
                for (Object str : arr)
        ...
        list.getSelectionModel().selectedItemProperty()
                .addListener((ov, oldvalue, newvalue) -> {
                            switch (newvalue.toString()) {
                                case "RSD (preordine)" -> zonaTextArea.appendText("\n- " + newvalue + ": " + arb.RSD().stream().map(Nod::toString).collect(Collectors.joining(" ")));
                                case "SRD (inordine)" -> zonaTextArea.appendText("\n- " + newvalue + ": " + arb.SRD().stream().map(Nod::toString).collect(Collectors.joining(" ")));
                                case "SDR (postordine)" -> zonaTextArea.appendText("\n- " + newvalue + ": " + arb.SDR().stream().map(Nod::toString).collect(Collectors.joining(" ")));
                                case "Numar nivele" -> {
                                    zonaTextArea.appendText("\n- " + newvalue + ": \n" + BinaryTreePrinter.printNode(arb.primul()));
                                    BTreePrinter.printNode(arb.primul());
                                }
                                case "Info" -> zonaTextArea.appendText("\nProgram: Arbore binar de cautare. Autor: Gherasim Daniel Adrian 3131B");
                            }
                        }
                );
        return list;
